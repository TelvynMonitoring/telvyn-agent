...
func TestDatabaseProfileKeepsLinuxKindAndAdvertisesDatabaseCapabilities(t *testing.T) {
	...
	t.Setenv("ISPWATCH_DATABASE_INSTALLATION_ID", "12345678-aaaa-bbbb-cccc-dddddddddddd")
	...
	if got := databaseCollectorName("postgres-01", databaseInstallationID()); got != "postgres-01 · banco · 12345678" {
		t.Fatalf("collector name=%q", got)
	}
}

func TestDatabaseProfileWithoutInstallationIDKeepsPostgresIdentity(t *testing.T) {
	t.Setenv("ISPWATCH_AGENT_PROFILE", "database")
	...
	if got := databaseCollectorName("postgres-01", databaseInstallationID()); got != "postgres-01 · PostgreSQL" {
		t.Fatalf("collector name=%q", got)
	}
}
